...
}: { label: string; value: string | number | null; color: string; onClick?: () => void }) {
  return (
    <div
...
function KpiSmall({
  label, value, color = 'text-hud-text', onClick,
}: { label: string; value: string | number | null; color?: string; onClick?: () => void }) {
  return (
    <div
      role={onClick ? 'button' : undefined}
...
function PhoneStat({ label, value, color = 'text-hud-text' }: { label: string; value: string | number | null; color?: string }) {
  return (
...
export default function SalesDashboard() {
  ...
  useEffect(() => {
    fetch('/api/settings')
      .then((r) => r.json())
      .then((cfg) => {
        if (cfg?.dashboardSales) setWidgetConfigs({ ...defaultWidgetConfigs, ...cfg.dashboardSales });
  ...
  useEffect(() => {
    const params = new URLSearchParams({ from: dateRange.from, to: dateRange.to, dashboard: 'sales' });
    ...
    async (newConfigs: WidgetConfigs) => {
      ...
      try {
        ...
        await fetch('/api/settings', {
          ...
          body: JSON.stringify({ ...current, dashboardSales: newConfigs }),
        });
      } catch {}
      try {
        const params = new URLSearchParams({ from: dateRange.from, to: dateRange.to, dashboard: 'sales' });
